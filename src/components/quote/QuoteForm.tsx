...
import type { QuoteFormValues } from '../../schemas/quoteSchema';
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <section className="bg-brand-card border border-brand-border rounded-xl p-4 sm:p-5">
        <h3 className="font-head font-bold text-brand-white text-sm mb-4">Quote Details</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <div>
            <label className="field-label">Quote Number</label>
            <input {...register('quoteNumber')} className="input-field font-head" />
            {errors.quoteNumber && <p className="field-error">{errors.quoteNumber.message}</p>}
          </div>
          <div>
      ...
      <section className="bg-brand-card border border-brand-border rounded-xl p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 gap-3 mt-4">
          <div>
            ...
            <input {...register('clientSnapshot.companyName')} className="input-field" placeholder="Client company name" autoComplete="organization" />
            {errors.clientSnapshot?.companyName && <p className="field-error">{errors.clientSnapshot.companyName.message}</p>}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="field-label">Contact Name</label>
              <input {...register('clientSnapshot.contactName')} className="input-field" autoComplete="name" />
            </div>
            <div>
              <label className="field-label">Email</label>
              <input {...register('clientSnapshot.email')} type="email" className="input-field" autoComplete="email" inputMode="email" />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="field-label">Phone</label>
              <input {...register('clientSnapshot.phone')} className="input-field" autoComplete="tel" inputMode="tel" />
            </div>
            <div>
              <label className="field-label">Address / Country</label>
              <input {...register('clientSnapshot.address')} className="input-field" autoComplete="street-address" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-card border border-brand-border rounded-xl p-4 sm:p-5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between rounded-lg p-4" style={{ background: 'rgba(255,107,0,0.12)', border: '1px solid rgba(255,107,0,0.25)' }}>
            <span className="font-head text-xs uppercase tracking-wider" style={{ color: '#FF6B00' }}>Total Quote</span>
      ...
      <section className="bg-brand-card border border-brand-border rounded-xl p-4 sm:p-5">
        <h3 className="font-head font-bold text-brand-white text-sm mb-3">Notes / Terms</h3>
        <textarea
          {...register('notes')}
          rows={3}
          className="input-field resize-none"
          placeholder="Validity, assumptions, and terms..."
        />
      </section>
    </div>
  );
